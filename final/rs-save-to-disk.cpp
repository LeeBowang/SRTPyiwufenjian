...
int main(int argc, char * argv[]) try
{
	...
	int res = connect(s_server, (struct sockaddr*)&server_addr, sizeof(server_addr));
	if (res == -1)
	{
		cout << "连接失败" << endl;
		exit(-1);
	...
	while(1)
	{
		...
		recv_len = read(s_server, recv_buf, 100);
		if (recv_len < 0) {
			cout << "接受失败！" << endl;
			break;
		}
		...
		read(s_server, recv_buf, 100);//x坐标相对值
		//cout << "接收成功！" << endl;
		x_temp = atof(recv_buf);
		//recv(s_server, recv_buf, 100, 0);//y坐标相对值
		read(s_server, recv_buf, 100);//y坐标相对值
		//cout << "接受成功！" << endl;
		...
		while(dist_to_center == 0)
		{
		rs2::frameset frames = pipe.wait_for_frames();
		rs2::depth_frame depth = frames.get_depth_frame();
		width = depth.get_width();
		height = depth.get_height();
		dist_to_center = depth.get_distance(width * x_temp, height * y_temp);
		}
		int dist = dist_to_center * 10000;//方便传输
		cout << dist << endl;
		//_itoa(dist, send_buf, 10);
		sprintf(send_buf, "%d", dist);
		int num = 0;
		int dist_temp = dist;
		while (dist_temp)
		{
